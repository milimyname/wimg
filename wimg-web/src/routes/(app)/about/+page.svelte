...
<section class="space-y-6">
  ...
  <div class="flex items-center gap-3">
    ...
    <h2 class="text-2xl font-display font-extrabold text-(--color-text)">Über wimg</h2>
  </div>

  <!-- Hero -->
  <div class="flex flex-col items-center text-center pt-4 pb-2">
    <div class="relative mb-5">
      <div class="w-20 h-20 rounded-2xl bg-(--color-text) flex items-center justify-center shadow-lg">
        <svg class="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
        </svg>
      </div>
      <div class="absolute -bottom-1.5 -right-1.5 w-7 h-7 bg-emerald-500 rounded-full border-[3px] border-(--color-bg) flex items-center justify-center">
    ...
    <h3 class="text-2xl font-display font-extrabold text-(--color-text) mb-1">wimg</h3>
    <p class="text-(--color-text-secondary) leading-snug max-w-[260px]">
      Persönliche Finanzverwaltung. Lokal. Privat. Offen.
    </p>
  </div>
  ...
  <div class="text-center px-2">
    <p class="text-(--color-text-secondary) text-sm leading-relaxed">
      Ein Open-Source-Projekt von <span class="font-semibold text-(--color-text)">Komiljon Maksudov</span>.
      Gebaut mit <span class="font-medium text-(--color-text)">Zig</span>,
      <span class="font-medium text-(--color-text)">Svelte 5</span> und
      <span class="font-medium text-(--color-text)">SQLite</span>.
    </p>
  </div>
  ...
  <div class="bg-emerald-50 p-4 rounded-2xl flex items-center gap-4">
    ...
    <div>
      <h4 class="font-bold text-(--color-text) text-sm">Privatsphäre zuerst</h4>
      <p class="text-xs text-(--color-text-secondary)">Keine Werbung. Kein Tracking. Niemals.</p>
    </div>
  </div>
  ...
  <a
    ...
    class="flex items-center justify-center gap-2 w-full py-3.5 border-2 border-(--color-text) text-(--color-text) font-bold text-sm rounded-2xl hover:bg-(--color-text) hover:text-white transition-colors active:scale-[0.98]"
  >
  ...
  <!-- FAQ -->
  <div>
    <h3 class="text-xl font-display font-extrabold text-(--color-text) mb-4 flex items-center gap-2">
    ...
    <div class="space-y-3">
      {#each faqs as faq}
        <details class="group bg-white rounded-2xl shadow-sm overflow-hidden">
          <summary class="flex cursor-pointer items-center justify-between p-4 list-none select-none">
            <span class="font-semibold text-sm text-(--color-text) pr-4">{faq.q}</span>
            <svg
          ...
          <div class="px-4 pb-4 text-sm text-(--color-text-secondary) leading-relaxed">
            {faq.a}
          </div>
        </details>
      {/each}
  ...
  <!-- Footer -->
  <footer class="flex flex-col items-center gap-3 pt-4 pb-6 text-(--color-text-secondary)">
    <a href={RELEASES_URL} target="_blank" rel="noopener noreferrer" class="text-xs font-bold text-amber-600 hover:underline">
      Was ist neu?
    </a>
    <span class="text-xs font-mono opacity-60">Version {APP_VERSION}</span>
  </footer>
</section>
