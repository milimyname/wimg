...
- **Phase 6.7** — FinTS Reliability + Background Sync (Planned): Add protocol regression tests (TAN submit/task reference, HITAN/HITANS dispatch, HIKAZ assembly, touchdown pagination), implement iOS background refresh for no-TAN paths (`3076`/`nochallenge`) with pending-action resume when TAN is required, and improve import result UX (pages fetched, imported/duplicates, clearer partial-sync messaging).
