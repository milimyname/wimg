...
class DrawerStore {
  ...
  // In-place mutations — no new Map, no reactivity cascade
  ...
  /** Sorted list of open drawers by open time (oldest first). */
  ...
  /** Stack position (0 = backmost). -1 if not open. */
  depth(id: string): number {
    const sorted = this.openDrawers();
    return sorted.findIndex((e) => e.id === id);
  }

  /** How many open drawers are above this one? */
