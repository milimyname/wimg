...
// Global embedding model instance (loaded on demand)
var embed_model: ?embed_mod.EmbedModel = null;
var embed_model_data: ?[]const u8 = null; // Pointer to GGUF data in WASM memory
...
/// JS callback for embedding progress (WASM only).
extern fn js_embed_progress(done: u32, total: u32) void;

/// Load an embedding model from GGUF data already in WASM memory.
/// The data pointer must remain valid (caller keeps the allocation).
/// Returns 0 on success, -1 on error.
export fn wimg_load_model(data: [*]const u8, len: u32) i32 {
    log("wimg_load_model: loading {d} bytes", .{len});

    const model_data = data[0..len];
    embed_model_data = model_data;

    embed_model = embed_mod.loadModel(model_data) catch |err| {
        setError("wimg_load_model: failed: {s}", .{@errorName(err)});
        embed_model_data = null;
        return -1;
    };
...
/// Embed a single text string. Returns length-prefixed JSON with the 384-dim vector.
export fn wimg_embed_text(text: [*]const u8, text_len: u32) ?[*]const u8 {
    const model = embed_model orelse {
        setError("wimg_embed_text: model not loaded", .{});
        return null;
    };
    ...
    const scratch = fba.allocator().alloc(f32, scratch_size) catch {
        setError("wimg_embed_text: scratch alloc failed", .{});
        return null;
    };
    ...
    for (out_vec, 0..) |v, i| {
        ...
        const formatted = std.fmt.bufPrint(json_buf[pos..], "{d:.6}", .{v}) catch break;
        pos += formatted.len;
    }
    json_buf[pos] = ']';
    pos += 1;

    return makeLengthPrefixed(json_buf[0..pos]);
}

/// Embed all transactions that don't have embeddings yet.
/// Returns count embedded, or -1 on error.
export fn wimg_embed_transactions() i32 {
    const model = embed_model orelse {
        setError("wimg_embed_transactions: model not loaded", .{});
        return -1;
    };

    var database = global_db orelse {
        setError("wimg_embed_transactions: database not initialized", .{});
        return -1;
    };
    ...
    const scratch_size = embed_mod.scratchSize();
    const scratch = fba.allocator().alloc(f32, scratch_size) catch {
        setError("wimg_embed_transactions: scratch alloc failed", .{});
        return -1;
    };
    defer fba.allocator().free(scratch);
    ...
    while (c.sqlite3_step(s) == c.SQLITE_ROW) {
        ...
        model.embed(prefixed_desc, &out_vec, scratch) catch |err| {
            log("embed failed: {s}", .{@errorName(err)});
            continue;
        };

        // Store as BLOB (384 * 4 = 1536 bytes)
        ...
        // Report progress per transaction
        if (is_wasm) {
            js_embed_progress(count, total);
        }
    }
    ...
    log("wimg_embed_transactions: embedded {d} transactions", .{count});
...
export fn wimg_smart_categorize() i32 {
    var database = global_db orelse {
        setError("wimg_smart_categorize: database not initialized", .{});
        return -1;
    };
    ...
    // Load categorized embeddings into memory
    const max_cat = 10000;
    const cat_embeddings = fba.allocator().alloc([384]f32, max_cat) catch {
        _ = sqlite_c.sqlite3_finalize(cat_stmt.?);
        setError("wimg_smart_categorize: alloc failed", .{});
        return -1;
    };
    defer fba.allocator().free(cat_embeddings);

    var cat_categories = fba.allocator().alloc(u8, max_cat) catch {
        _ = sqlite_c.sqlite3_finalize(cat_stmt.?);
        setError("wimg_smart_categorize: alloc failed", .{});
        return -1;
    };
    defer fba.allocator().free(cat_categories);

    var cat_is_income = fba.allocator().alloc(bool, max_cat) catch {
        _ = sqlite_c.sqlite3_finalize(cat_stmt.?);
        setError("wimg_smart_categorize: alloc failed", .{});
        return -1;
    };
    defer fba.allocator().free(cat_is_income);

    var n_cat: usize = 0;
    while (sqlite_c.sqlite3_step(cat_stmt.?) == sqlite_c.SQLITE_ROW) {
        if (n_cat >= max_cat) break;

        const cat_val: u8 = @intCast(sqlite_c.sqlite3_column_int(cat_stmt.?, 1));
        const blob_ptr = sqlite_c.sqlite3_column_blob(cat_stmt.?, 2) orelse continue;
        const blob_len: usize = @intCast(sqlite_c.sqlite3_column_bytes(cat_stmt.?, 2));
        const amount: i64 = sqlite_c.sqlite3_column_int64(cat_stmt.?, 3);

        if (blob_len != 384 * 4) continue; // Wrong size

        const float_bytes = blob_ptr[0..blob_len];
        const float_slice: *const [384]f32 = @ptrCast(@alignCast(float_bytes.ptr));
        cat_embeddings[n_cat] = float_slice.*;
        cat_categories[n_cat] = cat_val;
        cat_is_income[n_cat] = amount >= 0;
        n_cat += 1;
    }
    _ = sqlite_c.sqlite3_finalize(cat_stmt.?);
    ...
    var categorized: i32 = 0;

    while (sqlite_c.sqlite3_step(uncat_stmt.?) == sqlite_c.SQLITE_ROW) {
        const id_ptr = sqlite_c.sqlite3_column_text(uncat_stmt.?, 0) orelse continue;
        const id_len: usize = @intCast(sqlite_c.sqlite3_column_bytes(uncat_stmt.?, 0));
        const blob_ptr = sqlite_c.sqlite3_column_blob(uncat_stmt.?, 1) orelse continue;
        const blob_len: usize = @intCast(sqlite_c.sqlite3_column_bytes(uncat_stmt.?, 1));
        const uncat_amount: i64 = sqlite_c.sqlite3_column_int64(uncat_stmt.?, 2);
        const uncat_is_income = uncat_amount >= 0;

        if (blob_len != 384 * 4) continue;

        const uncat_vec: *const [384]f32 = @ptrCast(@alignCast(blob_ptr));

        // Find top-3 most similar categorized txns (same sign only: income↔income, expense↔expense)
        var top_sims: [3]f32 = .{ -1.0, -1.0, -1.0 };
        var top_cats: [3]u8 = .{ 0, 0, 0 };

        for (0..n_cat) |i| {
            // Only match against transactions with the same sign
            if (cat_is_income[i] != uncat_is_income) continue;

            const sim = quants_mod.dotProduct(uncat_vec, &cat_embeddings[i]);
            // Insert into top-3 if better
            if (sim > top_sims[2]) {
                top_sims[2] = sim;
                top_cats[2] = cat_categories[i];
                // Bubble sort
                if (sim > top_sims[1]) {
                    top_sims[2] = top_sims[1];
                    top_cats[2] = top_cats[1];
                    top_sims[1] = sim;
                    top_cats[1] = cat_categories[i];
                    if (sim > top_sims[0]) {
                        top_sims[1] = top_sims[0];
                        top_cats[1] = top_cats[0];
                        top_sims[0] = sim;
                        top_cats[0] = cat_categories[i];
                    }
                }
            }
        }

        // Decision: if top-1 > 0.85, assign directly
        if (top_sims[0] > 0.85) {
            database.setCategory(id_ptr, id_len, top_cats[0]) catch continue;
            categorized += 1;
            continue;
        }

        // If top-3 agree and avg > 0.7, assign
        if (top_cats[0] == top_cats[1] and top_cats[1] == top_cats[2]) {
            const avg = (top_sims[0] + top_sims[1] + top_sims[2]) / 3.0;
            if (avg > 0.7) {
                database.setCategory(id_ptr, id_len, top_cats[0]) catch continue;
                categorized += 1;
            }
    ...
    log("wimg_smart_categorize: categorized {d} transactions", .{categorized});
    return categorized;
}

/// Semantic search: find top-K most similar transactions to a query embedding.
/// Returns length-prefixed JSON array of {id, description, amount, category, similarity}.
export fn wimg_semantic_search(query: [*]const u8, query_len: u32, k: u32) ?[*]const u8 {
    const model = embed_model orelse {
        setError("wimg_semantic_search: model not loaded", .{});
        return null;
    };

    var database = global_db orelse {
        setError("wimg_semantic_search: database not initialized", .{});
        return null;
    };
    _ = &database;

    const raw_query = query[0..query_len];

    // e5 models require "query: " prefix for search queries
    var prefixed_buf: [1024]u8 = undefined;
    const prefix = "query: ";
    const prefixed_query = blk: {
        if (prefix.len + raw_query.len <= prefixed_buf.len) {
            @memcpy(prefixed_buf[0..prefix.len], prefix);
            @memcpy(prefixed_buf[prefix.len .. prefix.len + raw_query.len], raw_query);
            break :blk prefixed_buf[0 .. prefix.len + raw_query.len];
        }
        break :blk raw_query;
    };

    var query_vec: [384]f32 = undefined;

    // Allocate scratch buffer
    const scratch_size = embed_mod.scratchSize();
    const scratch = fba.allocator().alloc(f32, scratch_size) catch {
        setError("wimg_semantic_search: scratch alloc failed", .{});
        return null;
    };
    defer fba.allocator().free(scratch);

    model.embed(prefixed_query, &query_vec, scratch) catch |err| {
        setError("wimg_semantic_search: embed failed: {s}", .{@errorName(err)});
        return null;
    };
    ...
    const actual_k = @min(k, 50); // Cap at 50
    ...
    var results = fba.allocator().alloc(TopResult, actual_k) catch {
        setError("wimg_semantic_search: alloc failed", .{});
        return null;
    };
    defer fba.allocator().free(results);
    var n_results: usize = 0;

    // Initialize with -inf similarities
    for (results) |*r| {
        r.sim = -std.math.inf(f32);
    }

    while (sqlite_c.sqlite3_step(stmt.?) == sqlite_c.SQLITE_ROW) {
        const blob_ptr = sqlite_c.sqlite3_column_blob(stmt.?, 4) orelse continue;
        const blob_len: usize = @intCast(sqlite_c.sqlite3_column_bytes(stmt.?, 4));
        if (blob_len != 384 * 4) continue;

        const emb_vec: *const [384]f32 = @ptrCast(@alignCast(blob_ptr));
        const sim = quants_mod.dotProduct(&query_vec, emb_vec);

        // Check if this beats our worst result
        const worst_idx = if (n_results < actual_k) n_results else blk: {
            var min_idx: usize = 0;
            for (0..actual_k) |i| {
                if (results[i].sim < results[min_idx].sim) min_idx = i;
            }
            break :blk min_idx;
        };

        if (n_results < actual_k or sim > results[worst_idx].sim) {
            const idx = if (n_results < actual_k) blk: {
                n_results += 1;
                break :blk n_results - 1;
            } else worst_idx;

            results[idx].sim = sim;

            const id_ptr = sqlite_c.sqlite3_column_text(stmt.?, 0) orelse continue;
            const id_len: usize = @intCast(sqlite_c.sqlite3_column_bytes(stmt.?, 0));
            const copy_id_len = @min(id_len, 32);
            @memcpy(results[idx].id[0..copy_id_len], id_ptr[0..copy_id_len]);
            results[idx].id_len = copy_id_len;

            const desc_ptr = sqlite_c.sqlite3_column_text(stmt.?, 1) orelse continue;
            const desc_len: usize = @intCast(sqlite_c.sqlite3_column_bytes(stmt.?, 1));
            const copy_desc_len = @min(desc_len, 256);
            @memcpy(results[idx].desc[0..copy_desc_len], desc_ptr[0..copy_desc_len]);
            results[idx].desc_len = copy_desc_len;

            results[idx].amount = sqlite_c.sqlite3_column_int64(stmt.?, 2);
            results[idx].category = @intCast(sqlite_c.sqlite3_column_int(stmt.?, 3));
        }
    }
    ...
    const json = fba.allocator().alloc(u8, buf_size) catch {
        setError("wimg_semantic_search: json alloc failed", .{});
        return null;
    };
    ...
    return makeLengthPrefixed(json[0..pos]);
}

/// Get embedding status: total transactions, embedded count, model loaded.
/// Returns length-prefixed JSON.
export fn wimg_embedding_status() ?[*]const u8 {
    var database = global_db orelse {
        setError("wimg_embedding_status: database not initialized", .{});
        return null;
    };
...
/// Allocate memory for large data (e.g. embedding model) by growing WASM linear memory.
/// This bypasses the FBA which is too small for 157MB+ model files.
/// Returns a pointer to the allocated memory, or null on failure.
export fn wimg_alloc_model(size: u32) ?[*]u8 {
    if (!is_wasm) return null;

    const page_size: u32 = 65536; // 64 KB per WASM page
    const pages_needed = (size + page_size - 1) / page_size;

    // @wasmMemoryGrow returns the previous memory size in pages, or -1 on failure
    // First arg is memory index (always 0), second is pages to grow
    const prev = @wasmMemoryGrow(0, pages_needed);
    if (prev < 0) return null;

    const base: u32 = @intCast(@as(i64, prev) * page_size);
